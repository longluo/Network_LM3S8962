...
/*********************************************************************************************************
Macros that define the peripheral, port, and pin used for the OLEDDC panel control signal.
**********************************************************************************************************/
...
{
    0x03, 0xFD, 0x12, 0xe3,             // Unlock commands
    0x02, 0xAE, 0xe3,                   // Display off
    0x03, 0x94, 0x00, 0xe3,             // Icon off
    0x03, 0xA8, 0x5F, 0xe3,             // Multiplex ratio
    0x03, 0x81, 0xb7, 0xe3,             // Contrast
    0x03, 0x82, 0x3f, 0xe3,             // Pre-charge current
    0x03, 0xA0, RIT_INIT_REMAP, 0xe3,   // Display Re-map
    0x03, 0xA1, 0, 0xe3,                // Display Start Line
    0x03, 0xA2, RIT_INIT_OFFSET, 0xe3,  // Display Offset
    0x02, 0xA4, 0xe3,                   // Display Mode Normal
    0x03, 0xB1, 0x11, 0xe3,             // Phase Length
    0x03, 0xB2, 0x23, 0xe3,             // Frame frequency
    0x03, 0xB3, 0xe2, 0xe3,             // Front Clock Divider
    // 0x02, 0xB7, 0xe3                 // Set gray scale table.  App note uses default command:
    0x11, 0xB8, 1, 2, 3, 4, 5, 6, 8, 10, 12, 14, 16, 19, 22, 26, 30, 0xe3, 
                                        // This gray scale attempts some gamma correction to reduce the
                                        // the brightness of the low levels.
    0x03, 0xBB, 0x01, 0xe3,             // Second pre-charge period. App note uses value 0x04.
    0x03, 0xBC, 0x3f, 0xe3,             // Pre-charge voltage
    0x02, 0xAF, 0xe3,                   // Display ON
};

/*********************************************************************************************************
** Function name:           RITWriteCommand                                                             **
** Descriptions:            Write a sequence of command bytes to the SSD1329 controller.                **
...
{
    if (!HWREGBITW(&g_ulSSIFlags, FLAG_SSI_ENABLED))   // Return if SSI port is not enabled for RIT display.
    {
        return;
    }

    if (HWREGBITW(&g_ulSSIFlags, FLAG_DC_HIGH)) // See if data mode is enabled.
    {
        while(SSIBusy(SSI0_BASE))  // Wait until the SSI is not busy, meaning that all previous data has been transmitted.
        {
        }
        GPIOPinWrite(GPIO_OLEDDC_BASE, GPIO_OLEDDC_PIN, 0);    // Clear the command/control bit to enable command mode.
        HWREGBITW(&g_ulSSIFlags, FLAG_DC_HIGH) = 0;
    }

    while (0 != ulCount) // Loop while there are more bytes left to be transferred.
    {
        SSIDataPut(SSI0_BASE, *pucBuffer++);  // Write the next byte to the controller.
...
** Descriptions:            Write a sequence of data bytes to the SSD1329 controller.  .                **
**                          The data is written in a polled fashion; this function will not return      **
**                          until the entire byte sequence has been written to the controller.          **
** input parameters:        *parg                                                                       **
** output parameters:       none                                                                        **
...
RITWriteData(const unsigned char *pucBuffer, unsigned long ulCount)
{
    ...
    if (!HWREGBITW(&g_ulSSIFlags, FLAG_DC_HIGH)) // See if command mode is enabled.
    {
        while(SSIBusy(SSI0_BASE))   // Wait until the SSI is not busy, meaning that all previous commands
        {                           // have been transmitted.
        
        }

        GPIOPinWrite(GPIO_OLEDDC_BASE, GPIO_OLEDDC_PIN, GPIO_OLEDDC_PIN); // Set the command/control bit to enable data mode.
...
** Descriptions:            Clears the OLED display.                                                    **
...
RIT128x96x4Clear(void)
{
    ...
    *(unsigned long *)&g_pucBuffer[4] = 0;
    ...
    for (ulRow = 0; ulRow < 96; ulRow++)
    {
        ...
        // and the buffer hold 8 bytes, so 16 pixels are cleared at a time.
